...
export default function ReceiptPrintPage() {
  ...
  useEffect(() => {
    const fetchTransaction = async () => {
      try {
        const res = await fetch(`/api/admin/transactions?id=${params.id}`)
  ...
  // Auto print when print=1 and data is loaded
  useEffect(() => {
    if (!loading && transaction && shouldAutoPrint) {
  ...
  }, [loading, transaction, shouldAutoPrint])
  ...
  return (
    <div className="receipt-page">
      ...
      {!shouldAutoPrint && (
        <div className="print-buttons mb-4 flex items-center gap-4 p-4 bg-gray-100">
